...
function getMainBlocks(parsed) {
    return parsed.blocks.filter((block) => block.type === 'paragraph' || block.type === 'table');
}
function getHeaderStories(parsed) {
    return parsed.blocks.find((block) => block.type === 'headers')?.stories || [];
}
function getHeaderTextboxes(parsed) {
    return parsed.blocks
        .filter((block) => block.type === 'textboxes' && block.header)
        .flatMap((block) => block.items);
}
function getMainTextboxes(parsed) {
    return parsed.blocks
        .filter((block) => block.type === 'textboxes' && !block.header)
        .flatMap((block) => block.items);
}
function getHeaderShapes(parsed) {
    return parsed.blocks
        .filter((block) => block.type === 'shapes' && block.header)
        .flatMap((block) => block.items);
}
function getMainShapes(parsed) {
    return parsed.blocks
        .filter((block) => block.type === 'shapes' && !block.header)
        .flatMap((block) => block.items);
}
function buildChromeTemplates(parsed) {
    const templates = [];
...
function pickStoryTemplate(templates, kind, page, sections) {
    ...
    const preferredRoles = kind === 'header'
...
function pickTextboxTemplates(templates, kind, page) {
...
function createMeasureFrame(section) {
...
function sectionPageMetrics(section) {
    const widthPx = twipsToPx(section.page.pageWidthTwips) || 816;
    const heightPx = twipsToPx(section.page.pageHeightTwips) || 1056;
...
function applyPageBodyStyle(body, section) {
...
function shouldForceNewPageBeforeSection(prevSection, nextSection) {
    if (!prevSection || !nextSection)
        return false;
    if (prevSection.index === nextSection.index)
        return false;
...
function requiresInsertedBlankPage(nextSection, nextPhysicalPageNumber) {
    if (!nextSection)
        return false;
    if (nextSection.page.breakCode === 3)
        return nextPhysicalPageNumber % 2 !== 0;
    if (nextSection.page.breakCode === 4)
        return nextPhysicalPageNumber % 2 === 0;
    return false;
}
...
function buildPages(root, rendered) {
    ...
    if (!body)
        return [];
    const blocks = Array.from(body.children).filter((node) => node instanceof HTMLElement && node.classList.contains('msdoc-flow-block') && node.dataset.msdocFloating !== '1');
    if (!blocks.length)
        return [];
    const pages = [];
    ...
    for (const block of blocks) {
        ...
        if (blockSectionIndex !== currentSectionIndex && shouldForceNewPageBeforeSection(currentSection, nextSection)) {
            ...
            for (const existing of currentBlocks)
                measure.body.appendChild(existing.cloneNode(true));
        }
    ...
    const pushRuntimePage = (sectionIndex, blocksForPage, blank = false) => {
...
function shapeToPagePosition(shape, section) {
    const metrics = sectionPageMetrics(section);
...
function applyDynamicPageNumber(target, page) {
    ...
    if (!replaced)
        appendHtml(target, renderDynamicPageNumber(page));
}
function renderChromeContent(template, _page) {
    if (template.story)
        return renderBlockList(template.story.blocks);
    if (template.item)
        return renderBlockList(template.item.blocks);
    return '';
}
...
function buildImageAssetMap(rendered) {
    return new Map(rendered.assets.filter((asset) => asset.type === 'image').map((asset) => [asset.id, asset]));
}
function pickHeaderShapesForPage(shapes, kind, page, sections) {
    const section = sections[page.sectionIndex] || sections[0];
    const isEven = page.physicalPageNumber % 2 === 0;
    const isFirst = page.sectionPageIndex === 0;
    const preferredRoles = kind === 'header'
...
function createShapeAssetLayer(shape, asset) {
...
function renderShapeAssetOverlay(shape, asset, section, positionOverride) {
...
function findPageIndexForShape(pages, shape) {
    const targetSection = shape.sectionIndex ?? 0;
    let fallback = -1;
    for (let index = 0; index < pages.length; index += 1) {
        const page = pages[index];
        if (page.sectionIndex !== targetSection || page.blank || !page.blocks.length)
            continue;
        fallback = index;
        ...
        if (shape.anchorCp <= firstCpStart || shape.anchorCp <= lastCpEnd)
            return index;
    }
    if (fallback >= 0)
        return fallback;
...
function findAnchorBlockElement(bodyContent, anchorCp) {
    const blocks = Array.from(bodyContent.children).filter((node) => node instanceof HTMLElement);
    for (const block of blocks) {
        const cpEnd = Number(block.dataset.cpEnd || 0);
        if (!cpEnd || cpEnd < anchorCp)
            continue;
        return block;
    }
    return blocks.length ? blocks[Math.max(0, blocks.length - 1)] : null;
}
function shapeToBodyRelativePosition(shape, section, body, bodyContent) {
    const metrics = sectionPageMetrics(section);
...
function renderMainShapeOverlay(shape, textbox, asset, section, body, bodyContent) {
    ...
    if (asset)
        return renderShapeAssetOverlay(shape, asset, section, position);
    return null;
}
function renderPagedView(root, rendered) {
    const pagedView = root.querySelector('.msdoc-paged-view');
    if (!(pagedView instanceof HTMLElement))
        return;
    pagedView.innerHTML = '';
    const sections = getSections(rendered.parsed);
    ...
    const mainTextboxById = new Map(mainTextboxes.map((item) => [item.id, item]));
    const mainTextboxByShapeId = new Map(mainTextboxes.filter((item) => item.shapeId != null).map((item) => [item.shapeId, item]));
    for (const [pageIndex, page] of pages.entries()) {
        ...
        if (!page.blank) {
            for (const block of page.blocks)
                bodyContent.appendChild(block.cloneNode(true));
        }
        ...
        for (const kind of ['header', 'footer']) {
            for (const template of pickTextboxTemplates(chromeTemplates, kind, page)) {
                ...
                itemEl.style.width = `${position.width}px`;
                itemEl.style.minHeight = `${position.height}px`;
                if (template.item.shape.behindText)
                    itemEl.style.opacity = '0.92';
...
function installViewerRuntime(container, rendered) {
    ...
    const setView = (view) => {
        ...
        if (flowButton instanceof HTMLButtonElement)
            flowButton.setAttribute('aria-pressed', String(view === 'flow'));
        if (pagedButton instanceof HTMLButtonElement)
            pagedButton.setAttribute('aria-pressed', String(view === 'paged'));
    };
    const renderPages = () => renderPagedView(root, rendered);
    renderPages();
    setView('paged');
    const onClick = (event) => {
        const target = event.target;
        const view = target?.getAttribute?.('data-view');
        if (view === 'flow' || view === 'paged')
            setView(view);
    };
...
export function mountMsDoc(container, rendered) {
    ...
    const previousCleanup = container.__msdocCleanup;
    previousCleanup?.();
    container.__msdocCleanup = installViewerRuntime(container, rendered);
    return container;
}
...
export function createMsDocViewer(container, config = {}) {
    ...
    return {
        ...
        clear() {
            container.__msdocCleanup?.();
            delete container.__msdocCleanup;
            container.innerHTML = '';
            current = null;
        },
        destroy() {
