...
function readSttbfBkmk(tableBytes, fib) {
    const bytes = readBytes(tableBytes, fib.fcSttbfBkmk, fib.lcbSttbfBkmk);
    if (bytes.length < 6)
        return [];
...
function readFixedPlcEntries(tableBytes, fc, lcb, dataSize) {
    const bytes = readBytes(tableBytes, fc, lcb);
    if (!bytes.length || dataSize < 0 || bytes.length < 4)
        return [];
    const count = Math.floor((bytes.length - 4) / (4 + dataSize));
    if (count <= 0 || (bytes.length - 4) % (4 + dataSize) !== 0)
        return [];
...
function safeBookmarkId(name, index) {
 ...
 */
export function parseBookmarks(tableBytes, fib) {
    const names = readSttbfBkmk(tableBytes, fib);
    if (!names.length)
        return [];
    const starts = readFixedPlcEntries(tableBytes, fib.fcPlcfBkf, fib.lcbPlcfBkf, 4);
    const ends = readFixedPlcEntries(tableBytes, fib.fcPlcfBkl, fib.lcbPlcfBkl, 4);
    const bookmarks = [];
