...
function dopVariant(fib, lcb) {
    if (!lcb)
        return 'unknown';
    if (!fib.cswNew)
        return 'Dop97';
    if (fib.nFibNew === 0x00d9)
        return 'Dop2000';
    if (fib.nFibNew === 0x0101)
        return 'Dop2002';
    if (fib.nFibNew === 0x010c)
        return 'Dop2003';
 ...
 */
export function parseDop(tableBytes, fibRgFcLcb, fib) {
    const fc = fibRgFcLcb.fcDop;
    const lcb = fibRgFcLcb.lcbDop;
    ...
    const safeCount = (value, max) => (value && value > 0 && value <= max ? value : undefined);
    return {
