...
function setMeta(name, value, raw, bytes) {
    return { kind: 'section', name, value, raw, operandBytes: bytes };
}
function decodeSectionSprm(sprm, operandBytes) {
    switch (sprm) {
...
function decodeSectionGrpprl(buffer, startOffset, endOffset) {
    const properties = [];
...
function parseSepx(wordBytes, fcSepx) {
    if (!fcSepx || fcSepx === 0xffffffff || fcSepx < 0 || fcSepx + 2 > wordBytes.length)
        return [];
...
export function sectionPropsToPageSettings(properties) {
    const page = { ...DEFAULT_PAGE_SETTINGS };
    for (const property of properties) {
        switch (property.name) {
            case 'pageWidthTwips':
                page.pageWidthTwips = Math.max(144, Number(property.value) || page.pageWidthTwips);
                break;
            case 'pageHeightTwips':
                page.pageHeightTwips = Math.max(144, Number(property.value) || page.pageHeightTwips);
                break;
            case 'marginLeftTwips':
                page.marginLeftTwips = Math.abs(Number(property.value) || 0);
                break;
            case 'marginRightTwips':
                page.marginRightTwips = Math.abs(Number(property.value) || 0);
                break;
            case 'marginTopTwips':
                page.marginTopTwips = Math.abs(Number(property.value) || 0);
                break;
            case 'marginBottomTwips':
                page.marginBottomTwips = Math.abs(Number(property.value) || 0);
                break;
            case 'headerTopTwips':
                page.headerTopTwips = Math.max(0, Number(property.value) || 0);
                break;
            case 'footerBottomTwips':
                page.footerBottomTwips = Math.max(0, Number(property.value) || 0);
                break;
            case 'gutterTwips':
                page.gutterTwips = Math.max(0, Number(property.value) || 0);
                break;
            case 'columns':
                page.columns = Math.max(1, Number(property.value) || 1);
                break;
            case 'columnSpacingTwips':
                page.columnSpacingTwips = Math.max(0, Number(property.value) || 0);
                break;
            case 'titlePage':
                page.titlePage = Boolean(property.value);
                break;
            case 'orientation':
                page.orientation = property.value === 'landscape' ? 'landscape' : 'portrait';
                break;
            case 'breakCode':
                page.breakCode = Number(property.value) || 0;
                break;
            case 'restartPageNumber':
                page.restartPageNumber = Boolean(property.value);
                break;
...
export function readSections(wordBytes, tableBytes, fibRgFcLcb, mainStoryLength) {
    const entries = readFixedPlc(tableBytes, fibRgFcLcb.fcPlcfSed, fibRgFcLcb.lcbPlcfSed, 12);
    if (!entries.length) {
        return [{
                index: 0,
                cpStart: 0,
...
export function findSectionIndex(sections, cp) {
    for (const section of sections) {
        if (cp >= section.cpStart && cp < section.cpEnd)
