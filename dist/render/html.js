...
function firstRenderableTextStyle(block) {
    for (const node of block.inlines || []) {
        if (node.type === 'text' && normalizeRenderableText(node.text))
            return node.style;
        if (node.type === 'field' && normalizeRenderableText(node.displayText))
            return node.style;
        if ((node.type === 'image' || node.type === 'attachment') && node.style)
            return node.style;
    }
    return block.markStyle;
}
function listLabelStyleToCss(block) {
    const style = {};
    const paragraphTextStyle = firstRenderableTextStyle(block);
    if (paragraphTextStyle)
        Object.assign(style, inlineStyleToCss(paragraphTextStyle));
    ...
    if (block.list?.style)
        Object.assign(style, inlineStyleToCss(block.list.style));
    return style;
}
