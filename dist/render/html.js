...
function collectChromeEntries(headersBlock, headerTextboxes) {
    const map = new Map();
    ...
    for (const item of headerTextboxes) {
        ...
        const kind = isPageNumberLike(item.text || text) ? 'footer' : 'header';
        mergeChromeEntry(map, {
            kind,
            source: 'textbox',
            blocks: item.blocks,
...
function renderFloatingTextbox(shape, item) {
...
function renderFloatingShapePlaceholder(shape) {
...
function findNearestImageParagraphIndex(blocks, anchorCp, usedIds) {
    let bestIndex = null;
...
function placeFloatingArtifacts(mainBlocks, mainTextboxes, mainShapes) {
...
function renderMainContent(mainBlocks, mainTextboxes, mainShapes) {
...
function renderTextboxesAppendix(title, items) {
    if (!items.length)
        return '';
...
function renderShapesAppendix(title, shapes) {
    if (!shapes.length)
        return '';
