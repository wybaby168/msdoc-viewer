...
function createGuide(label: string, className: string, style: Partial<CSSStyleDeclaration>): HTMLDivElement {
...
function renderPagedView(root: HTMLElement, rendered: MsDocRenderResult): void {
  ...
  for (const [pageIndex, page] of pages.entries()) {
    ...
    if (headerStory) { appendHtml(headerBand, renderChromeContent(headerStory, page)); if (headerStory.pageNumberLike) applyDynamicPageNumber(headerBand, page); }
    ...
    if (footerStory) { appendHtml(footerBand, renderChromeContent(footerStory, page)); if (footerStory.pageNumberLike) applyDynamicPageNumber(footerBand, page); }
