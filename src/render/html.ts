...
function renderFieldNode(node: Extract<InlineNode, { type: 'field' }>): string {
  ...
  const href = sanitizeLinkHref(node.href);
  if (href) return `<a class="msdoc-link msdoc-field-link" href="${escapeHtml(href)}">${span}</a>`;
  return span;
}

function renderBookmarkAnchors(block: ParagraphBlock): string {
...
function renderInlineNodes(nodes: InlineNode[]): string {
  return nodes.map((node) => {
    ...
    if (node.type === 'field') return renderFieldNode(node);
...
function renderParagraphBlock(block: ParagraphBlock, options: { inline?: boolean; tableCell?: boolean } = {}): string {
  ...
  if (!inlineBody) Object.assign(paraStyle, paragraphMarkStyleToCss(block.markStyle));
  ...
  if (block.list) classNames.push('msdoc-list-paragraph');
  ...
  if (!options.inline) {
    ...
    if (block.bookmarkStarts?.length) attrs.push(` data-bookmark-starts="${escapeHtml(block.bookmarkStarts.map((bookmark) => bookmark.name).join(','))}"`);
