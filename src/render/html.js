...
function borderToCss(border) {
  ...
  const style = border.borderType === 6 ? 'double' : border.borderType === 3 ? 'dotted' : border.borderType === 2 ? 'dashed' : 'solid';
  const color = COLOR_INDEX_MAP[border.color] || '#666';
...
function inlineStyleToCss(styleState) {
  ...
  if (styleState.highlight?.index && HIGHLIGHT_COLORS[styleState.highlight.index]) style['background-color'] = HIGHLIGHT_COLORS[styleState.highlight.index];
...
function cellStyle(cell) {
  ...
  if (bottom) style['border-bottom'] = bottom;
  if (left) style['border-left'] = left;
  if (cell.meta?.shading?.color && COLOR_INDEX_MAP[cell.meta.shading.color]) {
    style['background-color'] = COLOR_INDEX_MAP[cell.meta.shading.color];
  }
  return style;
}
...
function tableStyle(block) {
  ...
  if (block.state?.leftIndent) style['margin-left'] = `${twipsToPx(block.state.leftIndent)}px`;
