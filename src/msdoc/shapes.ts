...
function decodeAnchorX(code: number): ShapeAnchorXOrigin {
  switch (code) {
...
function decodeAnchorY(code: number): ShapeAnchorYOrigin {
  switch (code) {
    case 0: return 'margin';
    case 1: return 'page';
...
function decodeWrapStyle(code: number): ShapeWrapStyle {
  switch (code) {
...
function decodeWrapSide(code: number): ShapeWrapSide {
  switch (code) {
    case 0: return 'both';
    case 1: return 'left';
    case 2: return 'right';
...
function parseSpaEntry(data: Uint8Array, storyCpBase: number, anchorCp: number, story: ShapeAnchorStoryKind): ShapeAnchorInfo | null {
  if (data.length < SPA_DATA_SIZE) return null;
 ...
 */
export function readShapeAnchors(
  ...
  const fc = story === 'header'
    ? (fibRgFcLcb.fcPlcSpaHdr as number | undefined)
    : (fibRgFcLcb.fcPlcSpaMom as number | undefined);
  const lcb = story === 'header'
    ? (fibRgFcLcb.lcbPlcSpaHdr as number | undefined)
    : (fibRgFcLcb.lcbPlcSpaMom as number | undefined);

  return readFixedPlc(tableBytes, fc, lcb, SPA_DATA_SIZE)
    .map((entry) => parseSpaEntry(entry.data, storyCpBase, entry.cpStart, story))
    .filter((entry): entry is ShapeAnchorInfo => Boolean(entry));
}
