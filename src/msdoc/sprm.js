...
function parseTInsertOperand(bytes) {
  ...
  return { cb, range: itc, dxaCol, cells };
}
...
export function getSprmOperandLength(buffer, offset, sprm) {
  ...
  switch (spra) {
    case 0: return 1; // toggle operand
    case 1: return 1;
    case 2: return 2;
    case 3: return 4;
    case 4: return 2;
...
function setMeta(kind, name, value, raw, bytes) {
  return { kind, name, value, raw, bytes };
}
...
export function decodeSprm(sprm, operandBytes) {
  ...
  switch (sprm) {
    ...
    case SprmCodes.sprmCHighlight: return setMeta('char', 'highlight', { index: bytes[0] ?? 0, color: HIGHLIGHT_COLORS[bytes[0] ?? 0] }, raw, bytes);
    ...
    case SprmCodes.sprmPShd: return setMeta('para', 'shading', bytes.slice ? bytes.slice() : bytes, raw, bytes);
    ...
    default:
      return { kind: group === 1 ? 'para' : group === 2 ? 'char' : group === 5 ? 'table' : 'unknown', name: `sprm_${sprm.toString(16)}`, value: bytes, raw: sprm, bytes };
  }
}
