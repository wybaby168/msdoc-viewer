...
function strokeAttributes(pen: PenState): string {
  ...
  if (pen.dasharray) attrs.push(`stroke-dasharray="${escapeHtml(pen.dasharray)}"`);
  return attrs.join(' ');
}
...
function fillAttributes(ctx: ParseContext): string {
  ...
  if (ctx.state.pen.dasharray && !ctx.state.pen.none) attrs.push(`stroke-dasharray="${escapeHtml(ctx.state.pen.dasharray)}"`);
