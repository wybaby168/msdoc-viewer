...
export function extractPictureAsset(dataStreamBytes, pictureOffset, options = {}) {
  ...
  const total = lcb > 0 && pictureOffset + lcb <= dataStreamBytes.length ? lcb : Math.min(dataStreamBytes.length - pictureOffset, options.maxPictureBytes || 8 * 1024 * 1024);
  const pictureChunk = dataStreamBytes.subarray(pictureOffset, pictureOffset + total);
  const segment = detectImageSegment(pictureChunk.subarray(Math.min(cbHeader || 68, pictureChunk.length)));
  if (!segment) {
    return {
  ...
  const start = Math.min(cbHeader || 68, pictureChunk.length) + segment.start;
  const end = Math.min(Math.min(cbHeader || 68, pictureChunk.length) + segment.end, pictureChunk.length);
...
function readObjectStorage(cfb, entry) {
  ...
  const objInfo = lower.get('\u0003objinfo') || lower.get('\x03objinfo') || lower.get(String.fromCharCode(3) + 'ObjInfo'.toLowerCase());
  const ole10 = lower.get('\u0001ole10native') || lower.get('\x01ole10native') || lower.get(String.fromCharCode(1) + 'Ole10Native'.toLowerCase());
  ...
  if (ole10) {
    const bytes = cfb.getStream(ole10);
    const nativeInfo = parseOle10Native(bytes || new Uint8Array(0));
    if (nativeInfo) {
