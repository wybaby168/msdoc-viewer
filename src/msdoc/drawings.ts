...
import { parseOfficeArtRecordHeader, type OfficeArtRecordHeader } from './objects.js';
import type {
  FibRgFcLcb,
  HeaderFooterRole,
  ImageAsset,
...
interface ParsedDrawingProperty {
  pid: number;
  fBid: boolean;
  fComplex: boolean;
  value: number;
...
interface ShapeScanInfo {
  shapeId: number;
  flags: number;
  shapeTypeCode?: number;
  bidRefs: ShapeBlipReference[];
  name?: string;
  description?: string;
}
...
interface DrawingContext {
  assets: ImageAsset[];
  assetByBlipIndex: Map<number, ImageAsset>;
  shapes: Map<number, ShapeScanInfo>;
  warnings: MsDocWarning[];
  fbseIndex: number;
  delayStreams: Array<{ name: string; bytes: Uint8Array }>;
}
...
function decodeUtf16(bytes: Uint8Array | undefined): string | undefined {
...
function decodeAnsi(bytes: Uint8Array | undefined): string | undefined {
  if (!bytes?.length) return undefined;
...
function detectMimeFromBlipType(blipType: number): string | null {
  switch (blipType) {
...
function extractBlipPayload(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader, extraMeta: Partial<ImageAssetMeta> = {}): PictureCandidate | null {
  ...
  if (
    header.recType === OFFICEART_BLIP_PNG
  ...
  if (
    header.recType === OFFICEART_BLIP_EMF
...
function createImageAsset(candidate: PictureCandidate, fbseIndex: number, fbseOffset: number, fbse: FbseInfo, sourceStream?: string): ImageAsset {
...
function parseFbseInfo(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader): FbseInfo | null {
  if (header.recType !== OFFICEART_FBSE || header.recLen < 36 || offset + header.size > bytes.length) return null;
  const payloadOffset = offset + 8;
...
function extractEmbeddedBlipFromFbse(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader, fbseIndex: number): ImageAsset | null {
  const fbse = parseFbseInfo(bytes, offset, header);
  if (!fbse) return null;
  const payloadOffset = offset + 8;
  const embeddedBlipOffset = payloadOffset + 36 + fbse.nameLength;
  if (embeddedBlipOffset + 8 > offset + header.size) return null;
...
function extractDelayedBlipFromFbse(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader, fbseIndex: number, streams: Array<{ name: string; bytes: Uint8Array }>): ImageAsset | null {
  const fbse = parseFbseInfo(bytes, offset, header);
  if (!fbse || !fbse.delayOffset) return null;
  for (const stream of streams) {
    if (!stream.bytes.length || fbse.delayOffset + 8 > stream.bytes.length) continue;
    const blipHeader = parseOfficeArtRecordHeader(stream.bytes, fbse.delayOffset);
    if (!blipHeader) continue;
...
function parseFoptProperties(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader): ParsedDrawingProperty[] {
  const properties: ParsedDrawingProperty[] = [];
  ...
  for (const property of properties) {
    if (!property.fComplex || property.value <= 0) continue;
 ...
 */
function scanOfficeArtRecords(
  bytes: Uint8Array,
  start: number,
  end: number,
  visitor: (offset: number, header: OfficeArtRecordHeader) => void,
): void {
...
function blipKindForPropertyId(id: number): ShapeBlipReference['kind'] {
  return id === PROP_PIB ? 'pib' : 'fillBlip';
}

function parseShapeContainer(bytes: Uint8Array, offset: number, header: OfficeArtRecordHeader): ShapeScanInfo | null {
  let shapeId = 0;
  let flags = 0;
  let shapeTypeCode: number | undefined;
  let name: string | undefined;
  let description: string | undefined;
  const bidRefs: ShapeBlipReference[] = [];
...
function parseDggBytes(bytes: Uint8Array, delayStreams: Array<{ name: string; bytes: Uint8Array }>): DrawingGroupInfo {
  ...
  scanOfficeArtRecords(bytes, 0, bytes.length, (offset, header) => {
    if (header.recType === OFFICEART_FBSE) {
      ...
      if (!asset) asset = extractDelayedBlipFromFbse(bytes, offset, header, context.fbseIndex, context.delayStreams);
      if (asset) {
    ...
    if (header.recType === OFFICEART_BSTORE_CONTAINER || header.recType !== OFFICEART_SP_CONTAINER) return;
    const shape = parseShapeContainer(bytes, offset, header);
    if (shape) context.shapes.set(shape.shapeId, shape);
  });

  const resolvedShapes = new Map<number, DrawingShapeInfo>();
  for (const shape of context.shapes.values()) {
    let blipRef: ShapeBlipReference | undefined;
    let imageAsset: ImageAsset | undefined;
 ...
 */
export function parseDrawingGroup(
  tableBytes: Uint8Array,
  wordBytes: Uint8Array,
  fibRgFcLcb: FibRgFcLcb,
  dataBytes: Uint8Array = new Uint8Array(0),
): DrawingGroupInfo {
  const fc = fibRgFcLcb.fcDggInfo as number | undefined;
  const lcb = fibRgFcLcb.lcbDggInfo as number | undefined;
...
function headerKindFromRole(role: HeaderFooterRole | undefined): 'header' | 'footer' | undefined {
 ...
 */
export function resolveHeaderAnchorBinding(
  roleWindows: Array<{ cpStart: number; cpEnd: number; sectionIndex?: number; role: HeaderFooterRole }>,
  anchorCp: number,
): HeaderAnchorBinding {
...
function entryToZeroBasedSection(sectionIndex: number | undefined): number | undefined {
  if (sectionIndex == null || sectionIndex <= 0) return undefined;
