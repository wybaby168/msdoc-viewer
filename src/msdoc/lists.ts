...
function normalizeLevelNumberFormat(nfc: number): string {
  switch (nfc) {
...
function toRoman(input: number, upper: boolean): string {
...
function toLetters(input: number, upper: boolean): string {
...
function formatNumber(value: number, nfc: number): string {
...
function parseLevel(bytes: Uint8Array, offset: number, level: number): { level: ListLevelDefinition; nextOffset: number } | null {
  ...
  const follow: ListLevelDefinition['follow'] = followByte === 0 ? 'tab' : followByte === 1 ? 'space' : 'none';
...
function parsePlfLst(tableBytes: Uint8Array, fib: FibRgFcLcb): ListDefinition[] {
  const bytes = readBytes(tableBytes, fib.fcPlfLst as number | undefined, fib.lcbPlfLst as number | undefined);
  ...
  for (let i = 0; i < count && offset + 28 <= bytes.length; i += 1) {
    ...
    for (let level = 0; level < 9; level += 1) styleIds.push(reader.i16(offset + 8 + level * 2));
...
function parsePlfLfo(tableBytes: Uint8Array, fib: FibRgFcLcb): ListOverride[] {
  const bytes = readBytes(tableBytes, fib.fcPlfLfo as number | undefined, fib.lcbPlfLfo as number | undefined);
  if (bytes.length < 4) return [];
  const reader = new BinaryReader(bytes);
  const count = reader.u32(0);
  let offset = 4;
  const overrides: ListOverride[] = [];
  ...
  for (const override of overrides) {
    const levels: ListOverrideLevel[] = [];
    for (let i = 0; i < override.overrideCount && offset + 8 <= bytes.length; i += 1) {
      ...
      offset += 8;
      let levelOverride: ListLevelDefinition | undefined;
      if (fFormatting) {
        const parsed = parseLevel(bytes, offset, level);
        if (parsed) {
...
export function parseLists(tableBytes: Uint8Array, fib: FibRgFcLcb): ListCollectionSummary {
  return {
...
function findLevel(lists: ListCollectionSummary, overrideIndex: number, levelIndex: number): { definition?: ListDefinition; level?: ListLevelDefinition; override?: ListOverride } {
...
function renderLevelTemplate(template: string, levelIndex: number, counters: number[], currentLevel: ListLevelDefinition): string {
...
export function applyListFormatting(paragraphs: ParagraphModel[], lists: ListCollectionSummary, options: ApplyListFormattingOptions = {}): void {
  ...
  for (const paragraph of paragraphs) {
    ...
    const overrideLevel = override?.levels.find((item) => item.level === levelIndex);
    if (!counters[levelIndex]) counters[levelIndex] = overrideLevel?.startAt ?? (effectiveLevel.startAt || 1);
    else counters[levelIndex] += 1;
    for (let i = levelIndex + 1; i < counters.length; i += 1) counters[i] = 0;
