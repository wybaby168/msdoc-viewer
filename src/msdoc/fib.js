...
export function parseFib(wordBytes) {
  ...
  const cswNew = reader.ensure(offset, 2) ? reader.u16(offset) : 0; offset += reader.ensure(offset, 0) ? 2 : 0;
...
export function parseFibBase(reader) {
  ...
  return {
    ...
    fDot: !!(flags & 0x0001),
    fGlsy: !!(flags & 0x0002),
    fComplex: !!(flags & 0x0004),
    fHasPic: !!(flags & 0x0008),
    cQuickSaves: (flags >> 4) & 0x0f,
    fEncrypted: !!(flags & 0x0100),
    fWhichTblStm: !!(flags & 0x0200) ? 1 : 0,
    fReadOnlyRecommended: !!(flags & 0x0400),
    fWriteReservation: !!(flags & 0x0800),
    fExtChar: !!(flags & 0x1000),
    fLoadOverride: !!(flags & 0x2000),
    fFarEast: !!(flags & 0x4000),
    fObfuscated: !!(flags & 0x8000),
    nFibBack: reader.u16(12),
    lKey: reader.u32(14),
    envr: reader.u8(18),
    fMac: !!(envFlags & 0x0001),
    fEmptySpecial: !!(envFlags & 0x0002),
    fLoadOverridePage: !!(envFlags & 0x0004),
  };
}
