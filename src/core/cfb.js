...
function buildPathMap(entries) {
  ...
  for (const entry of entries) {
    ...
    while (current) {
      ...
      current = current.parentId != null ? entries[current.parentId] : null;
    }
...
export function parseCFB(input, options = {}) {
  ...
  function getStream(identifier) {
    const entry = typeof identifier === 'string' ? pathMap.get(identifier) : identifier;
  ...
  function listChildren(path) {
    const entry = typeof path === 'string' ? pathMap.get(path) : path;
    if (!entry) return [];
    return (entry.children || []).map((id) => entries[id]).filter(Boolean);
  }
  ...
  return {
    ...
    findByName(name, startPath = '/') {
      const start = startPath === '/' ? root : pathMap.get(startPath);
      ...
      while (stack.length) {
        ...
        for (const childId of entry.children || []) stack.push(entries[childId]);
      }
      return null;
    },
  };
