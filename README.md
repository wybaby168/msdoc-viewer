...
- 分页 Viewer：按 section page settings 渲染纸张尺寸、页边距、页眉/页脚带、分栏和 odd/even page break 的空白页插入逻辑，可在分页和文档流之间切换；分页布局会保留 paragraph mark 的字符格式参与空段落高度计算，并按 LSPD 与 section document grid 信息处理行距，降低封面/目录类文档的纵向偏移
